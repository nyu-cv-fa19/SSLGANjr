...
class Trainer():
    ...
    def train_generator(self, fake_data, real_data, batch_size):
        ...
        rot_prob_fake = torch.sum(one_hot_label * g_fake_rot_prob,dim=1)
        rot_prob_real = torch.sum(one_hot_label * d_real_rot_prob,dim=1)
        fake_vec = torch.zeros(4,1)
        real_vec = torch.zeros(4,1)
        for i in range(4):
            fake_vec[i] = torch.mean(rot_prob_fake[i*batch_size:(i+1)*batch_size])
            real_vec[i] = torch.mean(rot_prob_real[i*batch_size:(i+1)*batch_size])
        
        dif = fake_vec - real_vec
        dist = 0
        for i in range(4):
        ...
        g_loss = g_loss + self.alpha * g_rot_loss
    ...
    def train_one_epoch(self, data_loader):
        for nth_batch, (real_batch, _ )in enumerate(data_loader):
            ...
            self.train_generator(fake_data, real_data,batch_size)
    ...
    def train(self, data_loader, epochs):
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1) % 20 == 0:
                name = str(epoch+1) + 'th'
                torch.save(self.G.state_dict(), './new_loss_G_' + name + '.pt')
                torch.save(self.D.state_dict(), './new_loss_D_' + name + '.pt')
