...
class Trainer():
    ...
    def train_generator(self, real_data, fake_data, batch_size):
        ...
        # calculate rotation loss
        # record jensen shannon distance
        js = []
        '''
        change the loss of the generator to avoid the problem of generating images with evident directional meaning.
        compare the distribution between real data and fake data under different direcitons.
        divergence: JS -> mean
        '''

        for i in range(batch_size*4):
            M = 0.5 * (g_fake_rot_prob[i] + d_real_rot_prob[i])
            kl1 = F.kl_div(g_fake_rot_prob[i],M)
            kl2 = F.kl_div(d_real_rot_prob[i],M)
            js.append(0.5 * (kl1 + kl2))
        
        js = torch.mean(torch.Tensor(js))    
        #g_fake_rot_loss = torch.mean(torch.sum(one_hot_label * torch.log(g_fake_rot_prob + 1e-12),dim=1))
        #d_real_rot_loss = torch.mean(torch.sum(one_hot_label * torch.log(d_real_rot_prob + 1e-12),dim=1))
        g_loss = g_loss + self.alpha * js
    ...
    def train_one_epoch(self, data_loader):
        for nth_batch, (real_batch, _ )in enumerate(data_loader):
            ...
            self.train_generator(real_data, fake_data, batch_size)
    ...
    def train(self, data_loader, epochs):
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1) % 20 == 0:
                name = str(epoch+1) + 'th'
                torch.save(self.G.state_dict(), './modified_G_' + name + '.pt')
                torch.save(self.D.state_dict(), './modified_D_' + name + '.pt') 
